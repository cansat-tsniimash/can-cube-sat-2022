...
static bool farm_1_process_frame(const map_data_t* md, const vc_data_t* vd) {
    //TODO
    return true;
}

static bool map_receive_octet(vc_receive_t* vcr, const map_data_t* md) {
    
    return true;
}

static bool map_receive_mapp(vc_receive_t* vcr, const map_data_t* md) {
    
    return true;
}

uint8_t extract_pvn(uint8_t byte) {
    uint8_t val = 0;
...
static void call_back(vc_receive_t* vcr) {
    if (extract_pvn(vcr->buffer.data[0]) == vcr->mapr.pvn) {
        ...
        uslp_event_mnf_t d = {.data = vcr->buffer.data, .size = vcr->buffer.index};
        uslp_event_handler(USLP_EVENT_MAPP_WRONG_PVN, &d);
    }
}
...
static void tfdz_mapp_to_data(vc_receive_t* vcr, const map_data_t* md, const vc_data_t* vd) {
    ...
    if (md->tfdf.pointer_fh_lo == 0xFFFF) {
        ...
        if (vcr->buffer.index == 0) {
            ...
            uslp_event_handler(USLP_EVENT_MAPP_NO_START, md);
            return;
        }
        ...
        if (vcr->buffer.size < vcr->buffer.index) {
            uslp_event_mnf_t d = {.data = vcr->buffer.data, .size = vcr->buffer.index};
            uslp_event_handler(USLP_EVENT_MAPP_BAD_LENGTH, &d);
...
static void tfdz_mapa_to_data(vc_receive_t* vcr, const map_data_t* md, const vc_data_t* vd) {
    if (md->tfdf.tfdz_rule == TFDZ_RULE_MAPA_START) {
        if (vcr->buffer.index != 0) {
            uslp_event_mnf_t d = {.data = vcr->buffer.data, .size = vcr->buffer.index};
            uslp_event_handler(USLP_EVENT_MAPA_NO_FINISH, &d);
        }
        clear_buffer(vcr);
