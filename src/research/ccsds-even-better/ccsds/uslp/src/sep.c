...
static transfer_frame_t _sep_get_from_mcf(sep_t* sep) {
    transfer_frame_t frame = {0};
    return frame;
}

static transfer_frame_t _sep_get_from_vc(sep_t* sep) {
...
static void _sep_forcing(sep_t* sep) {
    mx_node* pcmx = &sep->pc->mx;
...
static transfer_frame_t _sep_idle(sep_t* sep) {
...
static bool _sep_choose_path(sep_t* sep, bool forced) {
    mx_node* pcmx = &sep->pc->mx;
    mx_node* mcmx = mx_current_ready(pcmx);
    if (mcmx == 0) {
        if (!forced) {
...
static transfer_frame_t _sep_get_transfer_frame(sep_t* sep) {
    transfer_frame_t frame = {0};
    if (sep->path.mc == 0) {
        frame = _sep_idle(sep);
...
static uint16_t _sep_calc_size(const transfer_frame_t* frame) {
    uint16_t fsize = 0;
    fsize += 4;
...
static size_t _sep_serialize_transfer_frame(const transfer_frame_t* frame, uint8_t* data, size_t size) {
...
static void _sep_pop_transfer_frame(sep_t* sep) {
    if (sep->path.vc != 0) {
...
size_t sep_get_data(sep_t* sep, uint8_t* buffer, size_t size, bool forced) {
    ...
    uslp_event_handler(USLP_EVENT_SENT_FRAME, 0);
    return count;
}
